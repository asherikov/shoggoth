...
_required_vars = ["BM_MCP_URL", "OTEL_EXPORTER_OTLP_ENDPOINT"]
_missing = [v for v in _required_vars if v not in os.environ]
if _missing:
    print(f"Error: required environment variable(s) not set: {', '.join(_missing)}", file=sys.stderr)
    sys.exit(1)

BM_MCP_URL = os.environ["BM_MCP_URL"]
BM_MCP_TIMEOUT = int(os.environ.get("BM_MCP_TIMEOUT", "10"))
BM_TOOL_PREFIX = os.environ.get("BM_TOOL_PREFIX", "basic_memory-")
BM_OTEL_ENDPOINT = os.environ["OTEL_EXPORTER_OTLP_ENDPOINT"]
